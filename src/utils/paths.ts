...
const paths3D: Record<number, number[][]> = {
  ...
  8: [
    ...
    [2.0, 0.5, 2.0], [0.0, 1.0, 4.0], [-2.0, 0.5, 2.0], // top loop
    [0.0, 1.0, 0.0], // cross center (overpass)
    [2.0, 0.5, -2.0], [0.0, 0.0, -4.0], [-2.0, 0.0, -2.0], // bottom loop
  ...
  6: [
    ...
    [-2.0, 0.5, -1.0], // down
    [0.0, 0.0, -3.5], [2.0, 0.5, -2.0], // bottom curve
  ...
  5: [
    ...
    [-1.5, 0, 4.0], // left
    [-1.5, 0, 1.0], // down
    [1.5, 0, 1.5], [2.5, 0, 0.0], [2.5, 0, -1.5], // curve right and down
    [1.0, 0, -3.5], [-1.5, 0, -3.5], [-2.5, 0, -2.0] // curve left and up
  ],
  ...
  3: [
    ...
    [0.0, 0, 4.0], [2.5, 0, 2.5], [1.5, 0, 0.5], [0.0, 0, 0.5], // top curve
    [2.0, 0, -0.5], [2.5, 0, -2.5], [0.0, 0, -4.0], [-2.5, 0, -3.0] // bottom curve, end 
  ],

  // 2: Continuous, no crossing
  2: [
    [-3.0, 0, 2.0], [-1.0, 0, 4.0], [1.5, 0, 4.0], [3.0, 0, 2.0], // top curve
    [1.5, 0, 0.0], [-2.5, 0, -3.5], // diagonal
  ...
  1: [
    ...
    [-1.5, 0, -5.0], [1.5, 0, -5.0] // bottom serif for extra length
  ],
...
for (const [key, points] of Object.entries(paths3D)) {
  const vec3Points = points.map(p => new THREE.Vector3(p[0] * S, p[1] * H, p[2] * S));
  // tension 0.1 gives a very tight, well behaved curve without loops
  NUMBER_PATHS[parseInt(key)] = new THREE.CatmullRomCurve3(vec3Points, false, "catmullrom", 0.15);
}
