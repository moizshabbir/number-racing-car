...
export function UI() {
	const { status, time, speed, levels, currentLevelIndex, message } =
		useGameStore();
	...
	useEffect(() => {
		if (isCountdown) {
			setCountdownText("3");
			soundManager.playCountdownBlip();
			const t1 = setTimeout(() => {
				setCountdownText("2");
				soundManager.playCountdownBlip();
			}, 750);
				...
				<div className="absolute left-1/2 -translate-x-1/2 top-4">
					<div className="relative w-48 h-48 rounded-full border-4 border-white/20 bg-black/60 shadow-[0_0_30px_rgba(0,0,0,0.5)] flex items-center justify-center backdrop-blur-sm">
						...
						<div className="flex flex-col items-center mt-10">
							<span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1 italic">
								x1000 RPM
							</span>
							...
							<span className="text-sm text-cyan-400 font-bold italic">
								Km/h
							</span>
						</div>
			...
			<div className="absolute inset-0 flex items-center justify-center flex-col z-30 pointer-events-none">
				{isCountdown && countdownText && (
					<AnimatePresence mode="wait">
						...
						>
							<h1
								className="text-9xl font-black text-transparent italic"
				...
				<AnimatePresence>
					{message && (
						...
						>
							<h2
								className="text-8xl font-black text-transparent italic"
								style={{ WebkitTextStroke: "3px #00f2ff" }}
							>
								{message}
							</h2>
						</motion.div>
				...
				<div className="flex items-end gap-6 mr-8 drop-shadow-[0_0_15px_rgba(0,255,255,0.3)]">
					<div className="w-20 h-28 bg-gray-600/40 rounded-xl border-4 border-white/20 flex flex-wrap p-3 gap-2 content-start transform skew-x-[-10deg] backdrop-blur relative overflow-hidden">
						{[...Array(6)].map((_, i) => (
							<div
								key={i}
								className={`w-4 h-4 rounded-full ${speed < 0 ? "bg-red-500 shadow-[0_0_10px_rgba(255,0,0,1)]" : "bg-red-500/30"}`}
							></div>
						))}
					</div>
					<div className="w-16 h-36 bg-gray-500/40 rounded-xl border-4 border-white/30 flex flex-wrap p-3 gap-2 content-start transform skew-x-[-10deg] backdrop-blur">
						{[...Array(12)].map((_, i) => (
							<div
								key={i}
								className={`w-3 h-3 rounded-full ${speed > 10 ? "bg-green-400 shadow-[0_0_10px_rgba(0,255,0,1)]" : "bg-green-400/30"}`}
							></div>
						))}
					</div>
				</div>
