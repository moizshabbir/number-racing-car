...
export const Car = forwardRef<THREE.Group, CarProps>(({ trackRef }, ref) => {
    ...
    if (status === 'intro') {
        ...
        setTimeout(() => {
            if (useGameStore.getState().status === 'intro' && carRef.current) {
                ...
                const targetPos = new THREE.Vector3(carPos.x - 10, carPos.y + 4, carPos.z);
  ...
  useFrame((state, delta) => {
    ...
    if (carRef.current) {
      if (isPlaying) {
        ...
        // Cap speed
        speedRef.current = Math.max(Math.min(speedRef.current, maxSpeed), -maxSpeed / 2);

        // Steering
        if (Math.abs(speedRef.current) > 1) {
          const steerSpeed = 2.8 * (drift ? 1.8 : 1.0) * Math.sign(speedRef.current);
        ...
        const nextX = carRef.current.position.x + Math.sin(angleRef.current) * speedRef.current * dt;
        const nextZ = carRef.current.position.z + Math.cos(angleRef.current) * speedRef.current * dt;
        ...
        if (trackRef.current) {
          ...
          const trackMeshes = trackRef.current.children.filter(c => c.type === 'Mesh' && c.name === 'road-mesh');
        ...
        if (store.hasStarted) {
           ...
           } else if (store.checkpointsFound === 1 && carPos.distanceTo(c2) < dist) {
               setCheckpointsFound(2);
           } else if (store.checkpointsFound === 2 && carPos.distanceTo(c3) < dist) {
               setCheckpointsFound(3);
           }
           
           if (store.checkpointsFound >= 3 && carPos.distanceTo(c4) < 5 && status !== 'finish') {
               setStatus('finish');
      ...
      if (status === 'countdown' || status === 'playing') {
        ...
        if (isBoosting) {
           shakeOffset.set((Math.random() - 0.5) * 0.1, (Math.random() - 0.5) * 0.1, 0);
        }
  ...
  return (
    <group ref={carRef}>
      ...
      <spotLight position={[0.6, 0.4, 1.6]} angle={Math.PI / 4} penumbra={0.5} intensity={1.5} distance={60} color="#ffffff" castShadow target-position={[0, -1, 10]} />
      <spotLight position={[-0.6, 0.4, 1.6]} angle={Math.PI / 4} penumbra={0.5} intensity={1.5} distance={60} color="#ffffff" castShadow target-position={[0, -1, 10]} />
      
      {/* Headlights meshes */}
      <mesh position={[0.6, 0.3, 1.61]}>
        <planeGeometry args={[0.3, 0.15]} />
        <meshStandardMaterial emissive="#fff" emissiveIntensity={3} color="#fff" />
      </mesh>
      <mesh position={[-0.6, 0.3, 1.61]}>
        <planeGeometry args={[0.3, 0.15]} />
        <meshStandardMaterial emissive="#fff" emissiveIntensity={3} color="#fff" />
      ...
      <mesh position={[0.5, 0.4, -1.61]} rotation={[0, Math.PI, 0]}>
        ...
        <meshStandardMaterial emissive="#ff0000" emissiveIntensity={4} color="#ff0000" />
      </mesh>
      <mesh position={[-0.5, 0.4, -1.61]} rotation={[0, Math.PI, 0]}>
        <planeGeometry args={[0.5, 0.15]} />
        <meshStandardMaterial emissive="#ff0000" emissiveIntensity={4} color="#ff0000" />
      ...
      {/* Wheels */}
      {/* Front Left */}
      <mesh position={[-0.9, 0.25, 1.0]} rotation={[0, 0, Math.PI/2]} castShadow>
